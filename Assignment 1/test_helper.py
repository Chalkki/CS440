import cs440_assignment_1, algo_imp
import math
import time
...
def runfile(fileN):
    ...
    #astar
    start_a = time.time()
    grid, node_dict, path_a = algo_imp.main(x1, y1, x2, y2, grid, node_dict, row, col, "astar")
    end_a = time.time()

    #theta
    start_t = time.time()
    grid, node_dict, path_t = algo_imp.main(x1, y1, x2, y2, grid, node_dict, row, col, "theta")
    end_t = time.time()
